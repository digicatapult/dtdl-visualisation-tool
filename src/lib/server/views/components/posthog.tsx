...
export const PostHogScript = ({ req }: { req?: express.Request }): JSX.Element => {
  ...
  const bootstrapConfig = posthogId ? `,bootstrap:{distinctID:'${posthogId}'}` : ''
  ...
  const script = `
    ...
    posthog.init('${key}',{api_host:'${host}',autocapture:true,capture_pageview:true,capture_pageleave:true${bootstrapConfig}})
  `
