 ...
 * Prevents XSS by escaping backslashes, quotes, and newlines
 */
...
export const PostHogScript = ({ req }: { req?: express.Request }): JSX.Element => {
  ...
  const bootstrapConfig = posthogId ? `,bootstrap:{distinctID:'${escapeJs(posthogId)}'}` : ''
  ...
  const script = `
    ...
    posthog.init('${escapeJs(key)}',{api_host:'${escapeJs(host)}',autocapture:true,capture_pageview:true,capture_pageleave:true${bootstrapConfig}})
  `
