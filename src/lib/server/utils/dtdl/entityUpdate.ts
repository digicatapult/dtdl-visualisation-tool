import { z } from 'zod'
import { DtdlInterface, dtdlInterfaceBase, NullableDtdlSource, retainKeyOrder } from '../../../db/types.js'
...
const updateContentsValue = (
  ...
  const validInterface: z.infer<typeof schema> = retainKeyOrder(schema.loose()).parse(dtdlInterface)
...
const updateCommandRequestResponseValue = (
  ...
  const validFile: z.infer<typeof schema> = retainKeyOrder(schema.loose()).parse(file)
