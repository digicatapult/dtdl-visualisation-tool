...
test.describe('Share Ontology Link', () => {
  ...
  test('private ontology can be viewed correctly on another browser/github user edits enabled', async ({ browser }) => {
    ...
    const context1 = await browser.newContext({ storageState: join(tmpdir() + 'playwright/.auth/user1.json') })
    const projectName = test.info().project.name
    ...
    const context2 = await browser.newContext({ storageState: join(tmpdir() + 'playwright/.auth/user2.json') })
